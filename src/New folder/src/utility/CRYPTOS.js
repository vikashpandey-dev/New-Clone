...
const decryptJS = async (cipherData) => {
  if (process.env.enable_encription == 0) return cipherData;
  if (!cipherData) {
...
const EncryptJS = async (plain) => {
  if (process.env.enable_encription == 0) return plain;
...
exports.testEncrypt = async (plain) => {
  let randomIV = await randomString(16);
  var key = CryptoJS.enc.Utf8.parse("$P@mOu$0172@0r!P");
  var iv = CryptoJS.enc.Utf8.parse(randomIV);
  // var iv = CryptoJS.enc.Utf8.parse("@1O2j3D4e5F6g7P8");
...
export default { EncryptJS, decryptJS };
