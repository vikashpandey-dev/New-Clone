...
const Notification = (props) => {
  ...
  const { isOpen: isOpenStory, onOpen: onOpenStory, onClose: onCloseStory } = useDisclosure();
...
return ()=>clearTimeout(timeout)
  ...
  return (
    <div className="p-8">
      <Modal isOpen={isOpenNotification} onClose={onCloseNotification} isCentered>
        <ModalOverlay />
        <ModalContent borderRadius="10px">
          ...
          <ModalBody py={6}>
            <ModalNotification   EMPLI_D={selectedCheckboxes} alert={closepopup} />
          </ModalBody>
        </ModalContent>
      </Modal>
        ...
        <div className="flex justify-end w-1/2 gap-2">
          <div className="w-2/5">
            <InputGroup>
              ...
              <Input type="text" placeholder="Search, Name, Email, Id" height="10" value={search} onChange={(e) => setSearch(e.target.value)} />
            </InputGroup>
          </div>
          <Button
            p="2"
        ...
        >
          <Table variant="striped">
            <Thead>
              <Tr>
                <Th><input
                  onChange={handleSelectAllChange} className="cursor-pointer" type="checkbox" /></Th>
                <Th>Employee ID</Th>
                <Th>Name</Th>
                <Th>Email</Th>
                <Th>Contact</Th>
              ...
              {props.allowuser.length > 0 ? (
                props.allowuser.map((val) => {
                  return (
                    <>
                      <Tr>
                        <Td><input className="cursor-pointer" checked={selectedCheckboxes.includes(val.UserID)} onChange={() => handlecheckbox(val.UserID)} type="checkbox" /></Td>
                        <Td>{val.UserID}</Td>
